      ...
      try {
        ...
        setLogs((prevLogs) => {
          ...
          if (existingLogIndex >= 0) {
            ...
            setStatus(`Employee ${name} (ID: ${employeeId}) checked out at ${new Date(now).toLocaleTimeString()}`);
            return updatedLogs;
          } else {
            // Create a new log entry for Time In
            const newLog: Log = { name, employeeId, timeIn: now, timeOut: null };
            setStatus(`Employee ${name} (ID: ${employeeId}) checked in at ${new Date(now).toLocaleTimeString()}`);
          ...
          <tbody>
            {logs.map((log, index) => (
              <tr key={index} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="px-4 py-2">{log.timeIn ? new Date(log.timeIn).toLocaleString() : "-"}</td>
                <td className="px-4 py-2">{log.timeOut ? new Date(log.timeOut).toLocaleString() : "-"}</td>
              </tr>
            ))}
          </tbody>
        </table>
